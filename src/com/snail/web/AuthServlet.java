...
import com.snail.dao.SysAuthDao;
...
public class AuthServlet extends BaseHttpServlet {
	...
	SysAuthDao authDao = new SysAuthDao();
	...
	private void menuAction(HttpServletRequest request,
		...
		try {
			...
			String authIds = roleDao.getAuthIdsById(con,
					currentUser.getRoleId());
			JSONArray jsonArray = authDao.getAuthsByParentId(con, parentId,
					authIds);
	...
	private void authMenuAction(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		String parentId = request.getParameter("parentId");
		...
		try {
			...
			JSONArray jsonArray = authDao.getCheckedAuthsByParentId(con,
					parentId, authIds);
			ResponseUtil.write(response, jsonArray);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void authTreeGridMenuAction(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		String parentId = request.getParameter("parentId");
		Connection con = null;
		try {
	...
	private void saveAction(HttpServletRequest request,
		...
		SysAuth auth = new SysAuth(authName, authPath, authDescription, iconCls);
